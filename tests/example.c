...
static void *subscriber_thread(void *v) {
	ps_subscriber_t *s = ps_new_subscriber(10, PS_STRLIST("main.evt", "main.stopping"));
	sleep(1);
	...
	while (run) {
		...
		if (msg != NULL) {
			if (PS_IS_INT(msg)) {
				if (msg->rtopic != NULL) {
					PS_PUB_INT(msg->rtopic, msg->int_val + 1);
...
int main(void) {
	...
	ps_subscriber_t *sub = ps_new_subscriber(10, PS_STRLIST("thread.ready"));
	ps_msg_t *msg = ps_get(sub, 5000);
	if (msg == NULL || !PS_IS_BOOL(msg) || msg->bool_val != true) {
	...
	while (counter < 4) {
		...
		msg = PS_CALL_INT("main.evt", counter, 1000);
		if (msg != NULL && PS_IS_INT(msg)) {
			printf("Recv: %ld\n", msg->int_val);
		}
		ps_unref_msg(msg);
		counter++;
	}
	PS_PUB_BOOL("main.stopping", false);
